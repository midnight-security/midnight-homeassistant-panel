...
async def test_setup_entry_raises_repair_issue_when_backend_missing(hass):
    ...
    issue = ir.async_get(hass).async_get_issue(DOMAIN, MIDNIGHT_ALERTS_NOT_INSTALLED_ISSUE_ID)
...
async def test_setup_entry_succeeds_and_clears_issue_when_backend_present(hass):
    ...
    assert ir.async_get(hass).async_get_issue(DOMAIN, MIDNIGHT_ALERTS_NOT_INSTALLED_ISSUE_ID) is None
