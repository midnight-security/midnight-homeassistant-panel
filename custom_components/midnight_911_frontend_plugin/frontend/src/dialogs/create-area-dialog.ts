...
import { saveArea, deleteArea, fetchAreas, fetchSensors, subscribeConfigUpdates } from '../data/websockets';
import { AlarmoArea, Dictionary, AlarmoSensor, HomeAssistant } from '../types';
...
export class CreateAreaDialog extends SubscribeMixin(LitElement) {
  ...
  private saveClick(ev: Event) {
    ...
    // saveArea() falls back to a bare armed_away/armed_home default (with
    // every timer reset) whenever `modes` is omitted - this dialog only
    // collects a name, so the existing area's modes/timers must be carried
    // along explicitly here, or a plain rename would silently wipe them.
    if (this.area_id) data = { ...data, area_id: this.area_id, modes: { ...this.areas[this.area_id].modes } };
