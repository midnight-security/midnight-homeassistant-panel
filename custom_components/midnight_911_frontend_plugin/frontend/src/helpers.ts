...
export function handleError(err: any, ev: Event | HTMLElement) {
  // hass.callWS() rejects with the raw {code, message} the backend sent via
  // connection.send_error() (see home-assistant-js-websocket's
  // Connection.sendMessagePromise) - not the {body: {message}} shape a REST
  // (callApi) failure would have. Reading `err.body.message` here always
  // threw on a real backend error, so no one ever actually saw this dialog.
