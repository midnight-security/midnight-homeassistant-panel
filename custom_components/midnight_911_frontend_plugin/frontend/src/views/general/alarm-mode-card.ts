...
import { AlarmoModeConfig, EArmModes, Dictionary, AlarmoArea, AlarmoSensor, HomeAssistant } from '../../types';
import { fetchAreas, fetchSensors, saveArea, subscribeConfigUpdates } from '../../data/websockets';
...
export class AlarmModeCard extends SubscribeMixin(LitElement) {
  ...
  saveClick(ev: Event) {
    saveArea(this.hass, { area_id: this.selectedArea, name: this.areas[this.selectedArea].name, modes: this.data })
