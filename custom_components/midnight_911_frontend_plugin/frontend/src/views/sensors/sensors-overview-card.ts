...
import { computeName, navigate, sortAlphabetically } from '../../helpers';
    ...
    return {
      icon: {
        ...
        renderer: (data: AlarmoSensor) => {
          ...
          return data.area == noArea
            ...
            `
            : html`
              <ha-svg-icon .path=${icon} id="${formatId(data.entity_id, 'icon')}"></ha-svg-icon>
      ...
      name: {
        ...
        renderer: (data: AlarmoSensor & { name: string }) => html`
          <span id="${formatId(data.entity_id, 'name')}">
            ${data.name}
          </span>
          ${data.area == noArea
          ...
          <span class="secondary" id="${formatId(data.entity_id, 'name', 'secondary')}">
            ${data.entity_id}
          </span>
          ${data.area == noArea
      ...
      modes: {
        ...
        renderer: (data: AlarmoSensor) => html`
          <span id="${formatId(data.entity_id, 'modes')}">
            ${data.always_on
