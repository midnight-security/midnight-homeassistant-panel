...
export class UserEditorCard extends LitElement {
    ...
    return html`
      <ha-card>
        ...
        <alarmo-settings-row .narrow=${this.narrow}>
          ...
          <span slot="description">
            ${this.item && this.data.has_code
        ? localize('panels.codes.cards.edit_user.fields.code.description_keep_blank', this.hass.language)
  ...
  private saveClick(ev: Event) {
    ...
    else if (!this.item && (!data.code?.length || data.code.length < 4))
      showErrorDialog(ev, localize('panels.codes.cards.new_user.errors.no_code', this.hass.language));
    else if ((data.code || '').length && (data.code || '').length < 4)
      showErrorDialog(ev, localize('panels.codes.cards.new_user.errors.no_code', this.hass.language));
